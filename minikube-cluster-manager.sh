...
create_cluster() {
    ...
    # Build PostgreSQL image if Node.js, Deno, or Bun is selected
    if [ "$RUNTIME" == "node" ] || [ "$RUNTIME" == "deno" ] || [ "$RUNTIME" == "bun" ]; then
    ...
    # Copy PostgreSQL files if Node.js, Deno, or Bun is selected
    if [ "$RUNTIME" == "node" ] || [ "$RUNTIME" == "deno" ] || [ "$RUNTIME" == "bun" ]; then
